...
interface ContentPageProps {
    permisoId: { idPermiso: string }
}

// ---- Componente principal ----
const ContentPage: React.FC<ContentPageProps> = ({ permisoId }) => {
    ...
    useEffect(() => {
        ...
        changeTitle('Permisos - Editar')
        loadPermisoInfo()
        // eslint-disable-next-line
    }, [])

    const loadPermisoInfo = async () => {
        try {
            const { data, status, message } = await ConsumerAPI({
                url: `${process.env.NEXT_PUBLIC_API_URL}/api/permisos/${permisoId.idPermiso}/`
            })
            
            if (status === 'error') {
                console.error('Error al cargar permiso:', message)
                showLoader(false)
                Swal.fire({
                    title: 'Error',
                    text: message || 'No se pudo cargar la información del permiso',
                    icon: 'error'
                })
                return
            }
            ...
            console.error('Error en loadPermisoInfo:', error)
            showLoader(false)
            Swal.fire({
                title: 'Error',
                text: 'No se pudo cargar la información del permiso',
                icon: 'error'
            })
        }
    }
    ...
    const onSuccessResponse = () => {
        Swal.fire({
            ...
            text: 'Se ha actualizado el permiso correctamente',
            timer: 2000,
    ...
    const prepareDataAndSubmit = async () => {
        ...
        try {
            ...
            if (jsonResponse.status === 'error') {
                onErrorResponse(jsonResponse.message || 'Error al actualizar el permiso')
                return
            }
    ...
    return (
        <div className={styles.contentLayout}>
            ...
            <div className={styles.formPanel}>
                <h2 className={styles.sectionTitle}>Editar permiso</h2>
                <p className={styles.sectionDescription}>
                    <strong>Nota:</strong> Modifique el código, nombre y descripción del permiso.
                </p>
            ...
            <div className={styles.previewCard}>
                ...
                <div className={styles.previewInfo}>
                    <h3 className={styles.previewName}>
                        {inputValues.nombrePermiso || 'Permiso'}
                    </h3>
