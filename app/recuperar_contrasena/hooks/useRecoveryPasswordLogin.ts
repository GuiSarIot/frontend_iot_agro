...
import type { RecoveryPasswordRequestBody } from '../services/auth.types'

export function useRecoveryPasswordLogin() {
    ...
    const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        const sanitizedEmail = DOMPurify.sanitize(userEmail.current?.value ?? '')

        const body: RecoveryPasswordRequestBody = { userEmail: sanitizedEmail }
